...
class MoviesTestCase(unittest.TestCase):
    ...
    def test_post_movie_422(self):
        ...
        response = self.client().post(f"/movie", data=json.dumps(wrong_movie), content_type='application/json',
                                      headers=self.director_headers)
    ...
    def test_delete_movie(self):
        ...
        response = self.client().delete(f"/movie/{movie_id}", content_type='application/json',
                                        headers=self.assistant_headers)
        # Removal rejected
        self.assertEqual(response.status_code, 403)
    ...
    def test_patch_movie(self):
        ...
        movie = query_test_movie()
        movie_id = movie.id
        response = self.client().patch(f"/movie/{movie_id}", content_type='application/json', data=data,
                                       headers=self.director_headers)
        self.assertEqual(response.status_code, 200)
        # Check if the id of the movie in the response is the same one as inserted
